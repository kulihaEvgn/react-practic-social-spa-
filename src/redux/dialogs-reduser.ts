...
import { ADD_MESSAGE, CHANGE_TEXT_MESSAGE, DialogsActionsType } from "../types/dialogs/actions";
import { DialogsType, MessageType } from "../types/dialogs/Dialogs";
...
const createNewMessage = (state: DialogsType): MessageType => ({
    id: myId(),
    me: true,
    name: 'Ololosha',
...
export const dialogsReduser = (state = initialState, action: any): DialogsType => {
    switch (action.type) {
        case ADD_MESSAGE:
...
export const addMessageAC = (): DialogsActionsType => {
    return { type: ADD_MESSAGE }
}
export const changeTextMessageAC = (message: string): DialogsActionsType => {
